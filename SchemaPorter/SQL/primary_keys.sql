...
IF OBJECT_ID('tempdb..#ConstraintColumnUsage') IS NOT NULL DROP TABLE #ConstraintColumnUsage 
GO


SELECT
	 ', e.' + COLUMN_NAME AS CONSTRAINT_DEFINITION  
	,kcu.CONSTRAINT_CATALOG
	,kcu.CONSTRAINT_SCHEMA
	,kcu.CONSTRAINT_NAME
INTO #ConstraintColumnUsage 
...
SELECT 
	...
	,'pk_' + it.TABLE_NAME AS NEW_PRIMARY_KEY_NAME 
	...
	(
		...
		(
			(
				...
				FROM #ConstraintColumnUsage AS kcu 
				WHERE kcu.CONSTRAINT_CATALOG = tc.CONSTRAINT_CATALOG
				AND kcu.CONSTRAINT_SCHEMA = tc.CONSTRAINT_SCHEMA
				AND kcu.CONSTRAINT_NAME = tc.CONSTRAINT_NAME
