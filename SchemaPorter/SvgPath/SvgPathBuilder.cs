        ...
        : System.ComponentModel.TypeConverter
    {
        ...
        public static SvgPathSegmentList Parse(string path)
        {
            ...
            try
            {
                ...
                foreach (string commandSet in SplitCommands(path.TrimEnd(null)))
                {
            ...
            {
                System.Diagnostics.Trace.TraceError("Error parsing path \"{0}\": {1}", path, exc.Message);
            }
        ...
        private static System.Collections.Generic.IEnumerable<string> SplitCommands(string path)
        {
            var commandStart = 0;
            ...
            , object value, System.Type destinationType)
        {
            if (destinationType == typeof(string))
            {
                ...
                if (paths != null)
                {
                    var curretCulture = System.Globalization.CultureInfo.CurrentCulture;
                    try
                    {
                        ...
                        foreach (SvgPathSegment p in paths)
                        {
                            if (i != 0)
                                sb.Append(" ");
                        ...
                        // string s = string.Join(" ", paths.Select(p => p.ToString()).ToArray());
                    ...
                    finally
                    {
                        ...
                        System.Threading.Thread.CurrentThread.CurrentCulture = curretCulture;
                    }
                }
            }
